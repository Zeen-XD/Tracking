#!/usr/bin/python
# << YAHAHAHA MAU RECODE KONTOL
# << INI CODE PUNYA TUAN ZEEN
...
stderr.writelines(f"""{Gr}
          ...
          {Wh}[ + ]  C O D E   B Y  Mr.Zeen  [ + ] 
          {Wh}[ + ]  TELE te.me/RydenX  [ + ]  
        
    {Wh}[ 1 ] {Gr}Track Ip Addres
    {Wh}[ 2 ] {Gr}Ip Addres Kita
    {Wh}[ 3 ] {Gr}Track Pakai Nomor
    {Wh}[ 4 ] {Gr}Track Pakai Username
    {Wh}[ 0 ] {Gr}Exit
    {Wh}[100] {Gr} Pilih Salah Satunya Yah
""")
 ...
 `.. :            `. ``;
    ...
    try:
        def IP_Track():
            ...
            time.sleep(2)
            print(f"{Wh}\n IP ADDRES target       :{Gr}", ip)
            print(f"{Wh} Type IP         :{Gr}", ip_data["type"])
            print(f"{Wh} Benua         :{Gr}", ip_data["country"])
            print(f"{Wh} Kode negara    :{Gr}", ip_data["country_code"])
            print(f"{Wh} Kota            :{Gr}", ip_data["city"])
            print(f"{Wh} Kode Benua          :{Gr}", ip_data["region"])
